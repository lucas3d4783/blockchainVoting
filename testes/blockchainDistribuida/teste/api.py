...
#e só mostrar o bloco que tem um determinado index [requisição GET para http://127.0.0.1:8001/blocks/x (x sendo um inteiro)], 
...
@app.route('/blocks/quantity', methods=['GET']) 
def quantity(): # retorna todos os blocks
    try:
        o = select_node()
        quantity = '{"quantity":"' + str(o.get_chain_size()) + '"}'
        return quantity, 200 
    except:
...
@app.route('/blocks/quantity_of_nodes', methods=['GET']) 
def quantity_of_nodes(): # Retorna a quantity de processos do sistema 
    quant = '{"quantity_of_nodes":"' + str(len(nodes)) + '"}'
    return quant, 200 

#EXEMPLO:
# curl http://127.0.0.1:8001/blocks/1 | jq
@app.route('/blocks/<int:index>', methods=['GET']) 
def blocks_for_index(index): # filtrar blocks por index
    try:
        blockchain = json.loads(update_block_list())
        blocks = [block for block in blockchain if block['index'] == index] 
        if len(blocks) < 1: # caso não encontre nenhum bloco com o index informado
            return json.dumps({"erro": "Index nao encontrado!"}), 404
        return json.dumps(blocks), 200 # se encontrar retorna o bloco e um status de sucesso
    except:
